...
void CreditPlane::initHeader(unsigned header_character_size)
{
	std::string header = "DEVELOPED BY";
...
void CreditPlane::initContent(unsigned content_character_size)
{
	this->content.setFont(*this->font);
	this->content.setString("Sirisap Siripattnakul\nKMITL-63010978");
...
CreditPlane::CreditPlane(float x, float y, float width, float height, sf::Font* font, unsigned header_character_size, unsigned content_character_size)
